...
-- 4. Private poll uchun taklif qilingan users jadvali
CREATE TABLE
    IF NOT EXISTS `poll_invitations` (
        `id` CHAR(36) NOT NULL PRIMARY KEY,
        ...
        CONSTRAINT `poll_invitations_ibfk_1` FOREIGN KEY (`poll_id`) REFERENCES `polls` (`id`) ON DELETE CASCADE ON UPDATE CASCADE,
        CONSTRAINT `poll_invitations_ibfk_2` FOREIGN KEY (`user_id`) REFERENCES `users` (`id`) ON DELETE CASCADE ON UPDATE CASCADE
    ) ENGINE = InnoDB DEFAULT CHARSET = utf8mb4 COLLATE = utf8mb4_unicode_ci;

-- 5. Poll variantlari (options) jadvali
CREATE TABLE
...
-- 7. Verification kodlari jadvali
CREATE TABLE
...
-- 8. Foreign key cheklovlarini qayta yoqish
SET
